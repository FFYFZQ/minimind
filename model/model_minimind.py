...
def apply_rotary_pos_emb(q, k, cos, sin, position_ids=None, unsqueeze_dim=1):
    ...
    """
    在原始公式中使用的是相邻位置的旋转，但是在实际中为了访问连续内存，使用的x1和xdim/2+1
    """
...
def repeat_kv(x: torch.Tensor, n_rep: int) -> torch.Tensor:
    ...
    为了减少显存占用，key和value的个数通常少于head的个数
    """
...
class Attention(nn.Module):
    ...
    def forward(
        ...
        # TODO 位置写法存疑，是否传入的cos和sin经过了特殊处理
        ...
        if self.flash and seq_len > 1:
            if attention_mask is None or torch.all(
                attention_mask == 1
            ):  # 快速检查向量有没有填充
                ...
                extended_mask = (
                    1.0 - attention_mask.unsqueeze(1).unsqueeze(2)  # 1-是为了反转矩阵
                ) * float("-inf")
...
class MoEGate(nn.Module):
    ...
    def forward(self, hidden_states):
        ...
        if self.training and self.alpha > 0.0:
            ...
            aux_loss = 0
        return topk_idx, topk_weight, aux_loss  # 返回的是展平的weight和idx
...
class MOEFeedForward(nn.Module):
    ...
    def forward(self, x):
        identity = x
        orig_shape = x.shape
        bsz, seq_len, _ = x.shape  # (bs, seq, hidden)
        # 使用门控机制选择专家
...
class MiniMindModel(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        hidden_states = self.norm(hidden_states)  # 每一个layer结束之后要norm
